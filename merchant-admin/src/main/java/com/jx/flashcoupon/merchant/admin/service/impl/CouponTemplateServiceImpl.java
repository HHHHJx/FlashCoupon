...
import com.jx.flashcoupon.merchant.admin.common.constant.MerchantAdminRedisConstant;
...
import com.jx.flashcoupon.merchant.admin.dao.mapper.CouponTemplateMapper;
import com.jx.flashcoupon.merchant.admin.dto.req.CouponTemplateNumberReqDTO;
import com.jx.flashcoupon.merchant.admin.dto.req.CouponTemplatePageQueryReqDTO;
import com.jx.flashcoupon.merchant.admin.dto.req.CouponTemplateSaveReqDTO;
import com.jx.flashcoupon.merchant.admin.dto.resp.CouponTemplatePageQueryRespDTO;
import com.jx.flashcoupon.merchant.admin.dto.resp.CouponTemplateQueryRespDTO;
import com.jx.flashcoupon.merchant.admin.mq.event.CouponTemplateDelayEvent;
import com.jx.flashcoupon.merchant.admin.mq.producer.CouponTemplateDelayExecuteStatusProducer;
import com.jx.flashcoupon.merchant.admin.service.CouponTemplateService;
import com.jx.flashcoupon.merchant.admin.service.basics.chain.MerchantAdminChainContext;
...
import static com.jx.flashcoupon.merchant.admin.common.constant.CouponTemplateConstant.*;
import static com.jx.flashcoupon.merchant.admin.common.enums.ChainBizMarkEnum.MERCHANT_ADMIN_CREATE_COUPON_TEMPLATE_KEY;
...
public class CouponTemplateServiceImpl extends ServiceImpl<CouponTemplateMapper, CouponTemplateDO> implements CouponTemplateService {
    ...
    public void increaseNumberCouponTemplate(CouponTemplateNumberReqDTO requestParam) {
        ...
        // 校验增量非 0
        if (requestParam.getNumber() == null || requestParam.getNumber() == 0) {
            throw new ClientException("发行量变更值不能为 0");
        }

        // 记录优惠券模板修改前数据
        LogRecordContext.putVariable("originalData", JSON.toJSONString(couponTemplateDO));

        // 设置数据库优惠券模板库存发行量增减（正增负减），并避免负库存
        int changed = couponTemplateMapper.increaseNumberCouponTemplate(
                UserContext.getShopNumber(),
                requestParam.getCouponTemplateId(),
                requestParam.getNumber()
        );
        if (!SqlHelper.retBool(changed)) {
            throw new ServiceException("优惠券模板发行量变更失败");
        }

        // 同步变更优惠券模板缓存库存发行量（正增负减）
